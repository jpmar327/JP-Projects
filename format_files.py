...
def prep_file_paths(files_path_list):
    # filepath = os.path.join(main_folder_path, dirfolder, dirname)
    # count = 0
    # dsc_tracker = ''
    ...
    for key, value in files_path_list.items():
        ...
        for file in value:
            ...
            # files_path_dict[filepath] = os.path.join(main_folder_path, file)
    ...
    #         dsc_tracker = os.path.join(main_folder_path, dirfolder, dirname)
    #         count += 1
    #     file_type = dirname.split('.')[-1]
    # print(files_path_dict)

        # # # old_path = fr"{main_folder_path}/{dirfolder}/{dirname}"
        # # print(fr"{main_folder_path}/{dirfolder}/")
        # old_path = os.path.join(main_folder_path, dirfolder,dirname)
        # # # new_path = fr"{main_folder_path}/{subfolder}/{file_type}/{files_prefix}_{month}_{year}_{count}.{file_type}"
        # new_path = os.path.join(main_folder_path, subfolder, file_type, fr"{files_prefix}_{month}_{year}_{count}.{file_type}")
        # copy_path_dict[old_path] = new_path


    # for files in files_path_list:
    #     main_folder_path = files_path_list
    # if dsc_tracker == '' or filepath != dsc_tracker:
    #     dsc_tracker = os.path.join(main_folder_path, dirfolder, dirname)
    #     count += 1
    # file_type = dirname.split('.')[-1]
...
def prep_files_for_transfer(main_folder_path,subfolder,files_prefix,month,year):
    count = 0
    dsc_tracker = ''
    # copy_path_dict = {}
    # lookup_files(os.path.join(main_folder_path))
    ...
    for dirfolder in os.listdir(main_folder_path):
        if dirfolder == subfolder:
            print('FORMATTED')
            break
        # for dirname in os.listdir(os.path.join(main_folder_path,dirfolder)):
        #     files_path_list = lookup_files(os.path.join(main_folder_path,dirfolder,dirname))
        #     print(files_path_list)
        #     prep_file_paths(files_path_list)
            # print(os.listdir(os.path.join(main_folder_path,dirfolder)))
            # print(os.listdir(fr"{main_folder_path}\{dirfolder}"))
            # print(f"PATH NAME: {os.path.join(main_folder_path, dirfolder,dirsubfolder)}")
            # print(os.listdir(os.path.join(main_folder_path, dirfolder)))
            # print(dirname)
            # print(os.path.join(main_folder_path,dirfolder))
            # print("list",os.listdir(os.path.join(main_folder_path,dirfolder)))
            # for dirname in os.listdir(os.path.join(main_folder_path,dirfolder)):
...
def main():
    ...
    # files_type_list = return_files_types(main_folder_path)
    # copy_path_dict = prep_files_for_transfer(main_folder_path,subfolder,files_prefix,month,year)
    files_path_list = lookup_files(os.path.join(main_folder_path))
    ...
    # print(list(dict.fromkeys(list(copy_path_dict.values()))))
    # print(list(set([file_name.split('.')[-1].upper() for file_name in list(copy_path_dict.values())])))
    # print(list(copy_path_dict.values()))
    # create_file_types_folders(main_folder_path,subfolder,files_type_list)
